...
def _get_mfcc_log_spec_and_log_mel_spec(wav, preemphasis_coeff,
    ...
    product = np.dot(mel_basis.T, mel_basis)
    print(product)
    print(mel_basis.shape)
    # p_inv = np.dot(np.linalg.inv(np.dot(mel_basis.T, mel_basis)),
    #                mel_basis.T)
    # mag_pred = np.dot(p_inv, mel)
    # print(mag)
    # print(mag_pred)

    # Get mfccs
    db = librosa.amplitude_to_db(mel)
    dctm = librosa.filters.dct(hp.Default.n_mfcc, db.shape[0])
    mfccs = np.dot(dctm, db)
    # Log
    ...
    # self.y_log_spec = (y_log_spec - hp.min_log_spec) / \
    #     (hp.max_log_spec - hp.min_log_spec)

    return mfccs.T, mag.T, mel.T  # (t, n_mfccs), (t, 1+n_fft/2), (t, n_mels)


def spectrogram2wav(mag, n_fft, win_length, hop_length,
                    num_iters, phase_angle=None, length=None):
    assert(num_iters > 0)
...
def _get_wav_from_mfccs(mfccs, preemphasis_coeff,
                        n_fft, win_length, hop_length, n_wav):
